<template>
  <div class="max-h-screen min-h-screen flex flex-col justify-between">
    ...
    <div class="max-h-[calc(100vh-128px)] flex grow flex-row">
      ...
      <div
        class="grow overflow-y-auto scrollbar scrollbar-thumb-color-gray-9 scrollbar-track-color-gray-500 scrollbar-rounded scrollbar-radius-2 scrollbar-thumb-radius-4 scrollbar-track-radius-4 scrollbar-w-2"
      >
        <NuxtPage />
      </div>
    </div>
  ...
  try {
    ...
    if (listId) {
      if (!listId.match(/[\w-]{21}/)) {
        ...
        listStore.setList(lists[0] as List);
        if (characterId) {
          if (!characterId.match(/[\w-]{21}/)) {
            ...
            if (characters.length === 0) {
              ...
              characterStore.setCharacter(characters[0] as Character);
            }
          }
        }
      }
...
onMounted(() => {
  ...
  if (lsTutorial) {
    mainStore.tutorial = Number.parseInt(lsTutorial) as 1 | 2 | 3;
  }
  else {
